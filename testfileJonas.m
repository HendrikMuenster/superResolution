...
if color
    imageSequenceStart = imageSequenceSmall(:,:,:,1:numFrames);
else 
    imageSequenceStart = imageSequenceSmall(:,:,1:numFrames);
end
...
mainSuper.regUq = 0.3;                     % TV/TGV-2 exponent
mainSuper.regTGV = 0.5;                    % TV-TGV weight
mainSuper.alpha = [0.1,0.0006];%[0.1,0.01,0.001]; % regU weights
mainSuper.kOpts.delta  = [0.05,0];              % blur l2 penalties
mainSuper.kOpts.zeta   = [0.15,0];             % blur Tikh penalties

mainSuper.regV = 'Huber';                  % regV type
mainSuper.beta = [0.1,0.05];%[0.3,0.2,0.1]; % regV weights

mainSuper.gamma = [1,1];                       % Coupling Term weights
